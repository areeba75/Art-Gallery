...
const Services = () => {
  return (
    <div className="pb-[100px] pt-[40px]">
      <h2 className="text-center p-[12px] my-[26.56px]">
      ...
      <div className="flex flex-col lg:flex-row gap-8 justify-center px-5 sm:px-[56px] lg:w-[1083px] xl:w-[1300px] m-auto">
        {cards.map((card, index) => (
          ...
          >
            <h4 className="text-[#F6C934]">{card.heading}</h4>
            <p className="leading-6">{card.text}</p>
            <button className="text-[18px] my-[7px] md:text-[20px] custom-underline text-yellow-400">
              {card.button}
            </button>
          </div>
        ))}
